...
export function RolePermissionsForm({ allPermissions, selectedPermissionIds = [], idPrefix }: RolePermissionsFormProps) {
  ...
  // 3. If it contains selected permissions (useful for edit mode)
  const isModuleOpen = (module: string, permsInModule: Permission[]) => {
    ...
    const hasSelected = permsInModule.some(p => selectedIds.includes(p.id));
                      ...
                      <div key={resource}>
                        ...
                        <div className="grid grid-cols-2 gap-2">
                          {perms.map((p) => {
                            ...
                            return (
                            <div key={p.id} className="group flex items-center w-fit space-x-2">
                              <input
                                type="checkbox"
                                id={id}
                                checked={selectedIds.includes(p.id)}
                                onChange={(e) => handleTogglePermission(p.id, e.target.checked)}
                                className="h-4 w-4 rounded border-gray-300 text-primary/85 group-hover:text-primary focus:ring-primary transition-colors cursor-pointer"
                              />
                              <TooltipProvider>
                                <Tooltip>
                                  <TooltipTrigger asChild>
                                    <Label
                                      htmlFor={id}
                                      className="font-normal text-[11px] text-primary/85 group-hover:text-primary cursor-pointer transition-colors flex items-center gap-1.5"
                                    >
                                      {p.action}
                                      {(p.resource === '*' || p.resource === "system-settings") && (
